# Home Assistant App: Bambuddy

![Supports aarch64 Architecture][aarch64-shield] ![Supports amd64 Architecture][amd64-shield]

This app integrates [BamBuddy](https://github.com/MagicMelody84/bambuddy) into Home Assistant — a self-hosted command center for Bambu Lab — from one A1 to a 40-printer farm. 

<strong>Your Printers. No Cloud. Your Rules.</strong>

It allows you to monitor and manage your prints directly from your Home Assistant instance.

[aarch64-shield]: https://img.shields.io/badge/aarch64-yes-green.svg
[amd64-shield]: https://img.shields.io/badge/amd64-yes-green.svg
